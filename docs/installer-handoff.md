...
| on-screen keyboard renderer (driven with fake frames, self-captured with PrintWindow) | 994x344 px at 125 %, five rows as specified, shifted glyphs while L2 is held, highlight tracking, glyph hints. NB windows created from a sandboxed non-interactive shell live on a separate desktop and are invisible to other processes -- capture from inside the process. |
