...
export const HELP: Record<string, string> = {
  ...
  touch_slide_horizontal: "Slide two fingers left or right across the touchpad without pressing it. Bound to a scroll action the wheel follows the fingers like a precision touchpad; bound to alt_tab, it opens the window switcher and steps through it as you slide; lift to switch.",
  touch_slide_vertical: "Slide two fingers up or down the touchpad without pressing it. Scrolls by default, proportional to the movement (Remote mode's scroll speed applies).",
  touch_swipe_up: "A quick two-finger flick up, pad not pressed. Kept for older configs: unbound by default now that the vertical slide scrolls.",
  touch_swipe_down: "A quick two-finger flick down, pad not pressed. Kept for older configs: unbound by default now that the vertical slide scrolls.",
  touch_pinch: "Two fingers moving apart or together, pad not pressed. Pinch zoom by default: a touch-injected pinch that zooms the page the way a precision touchpad does (the visual viewport, not the browser's zoom level).",
  touch_slide_horizontal_pressed: "Click the touchpad and, keeping it pressed, slide two fingers left or right. Alt+Tab by default. A contact that was clicked and then slid fires only this, never the unpressed slide.",
  touch_swipe_up_pressed: "Click the touchpad and, keeping it pressed, swipe two fingers up. Task View by default.",
  touch_swipe_down_pressed: "Click the touchpad and, keeping it pressed, swipe two fingers down. Minimize all by default.",
  touch_pinch_pressed: "Click the touchpad and, keeping it pressed, pinch or spread two fingers. Ctrl + wheel zoom by default (the app's own zoom level).",
...
export const KEY_LABELS: Record<string, string> = {
  ...
  touch_slide_vertical: "Two-finger slide (up/down)",
  touch_swipe_up: "Two-finger swipe up",
  touch_swipe_down: "Two-finger swipe down",
  touch_pinch: "Pinch / spread",
  touch_slide_horizontal_pressed: "Slide left/right, pad clicked",
  touch_swipe_up_pressed: "Swipe up, pad clicked",
  touch_swipe_down_pressed: "Swipe down, pad clicked",
...
export const GESTURE_FALLBACK: GestureRow[] = ([
  ...
  ["touch_slide_horizontal", "unpressed"], ["touch_slide_vertical", "unpressed"],
  ["touch_swipe_up", "unpressed"], ["touch_swipe_down", "unpressed"], ["touch_pinch", "unpressed"],
  ["touch_slide_horizontal_pressed", "pressed"], ["touch_swipe_up_pressed", "pressed"],
  ["touch_swipe_down_pressed", "pressed"], ["touch_pinch_pressed", "pressed"],
] as const).map(([key, group]) => ({ key, group, label: KEY_LABELS[key], help: HELP[key] }));

export const GESTURE_GROUPS: { id: string; title: string; hint: string }[] = [
  { id: "taps", title: "Taps & clicks", hint: "two fingers on the pad" },
  { id: "unpressed", title: "Slides, swipes & pinch", hint: "pad not pressed" },
  { id: "pressed", title: "While the pad is clicked", hint: "click, hold, then move two fingers" },
];
